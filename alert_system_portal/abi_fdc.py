...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 25 14:51:43 2021
...
ptho = f'{mainpth}/pts_fdc'
if not os.path.isdir(ptho):
    os.makedirs(ptho)
    print(f"Se creó: {ptho}")
else:
    print(f"Existe : {ptho}")
#%%
...
for id_hr in matches.keys():
    ...
    fileout = f'{ptho}/{id_hr}.csv'
    if not os.path.exists(fileout):
        df = grfuns.fdc2csv(gdata)
        df.to_csv(fileout,index=False)

#%%
lst2del = glob(f'{ptho}/*.csv')
lst2del.sort()
#%%
tme = []
...
for gp_key, gp_value in gpd_df:
    ...
    if minsecs == 0:
        listtmp = pdd['Temp'].to_list()
        
        lasttmp = pdd['Temp'].values[-1]#pdd['Temp'].values[-1]
        lastmsk = pdd['Mask'].values[-1]#pdd['Mask'].values[-1]
        lastpwr = pdd['Power'].values[-1]#pdd['Power'].values[-1]
        ...
        str1 = f"""( "type": "Feature", "properties": ( "name": "Fire Temp.", "units": "[C]", "Status": "{stat}", "data": {lasttmp} , "time_o": "{dtt}", "time_0": "{time0}", "time": "{lastac}", "lat": {lai}, "lon": {loi}, "serie": {listtmp}, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ {loi}, {lai} ] ) ),"""
        ftrs.append(str1)
if not ftrs:
    ftrs = ["""( "type": "Feature", "properties": ( "name": "Fire Temp.", "units": "[C]", "Status": null, "data": null , "time_o": null, "time_0": null, "time": null, "lat": 0, "lon": 0, "serie": null, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ 0, 0 ] ) ),"""]
...
file2up = f'{pthg}/pts_fdc.geojson'
f = open(file2up, 'w')
f.write(jsonf)
f.close()
