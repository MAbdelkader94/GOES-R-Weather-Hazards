...
ptho2 = f'{mainpth}/pts_adpS'
if not os.path.isdir(ptho2):
    os.makedirs(ptho2)
    print(f"Se creó: {ptho2}")
else:
    print(f"Existe : {ptho2}")
#%%
...
for id_hr in matches.keys():
    ...
    fileout = f'{ptho2}/{id_hr}.csv'
    if not os.path.exists(fileout):
        df = grfuns.adpS2csv(gdata)
        df.to_csv(fileout,index=False)
#%%
lst2del2 = glob(f'{ptho2}/*.csv')
lst2del2.sort()
#%%
tme = []
for n, fl in enumerate(lst2del2):
    pd0 = pd.read_csv(fl)
...
for gp_key, gp_value in gpd_df:
    ...
    if minsecs == 0:
        ...
        listdst = pdd['Smoke'].to_list()        
        lastdst = pdd['Smoke'].values[-1]
        
        lai = round(group['lat'].values[-1], 3)
        loi = round(group['lon'].values[-1], 3)
        
        currT = pdd['Smoke'].values[-1]
        if lastdst == 0:
            dststat = 'Inactive'
        else:
            dststat = 'Active'
        ...
        str1 = f"""( "type": "Feature", "properties": ( "name": "Smoke", "units": "[-]", "Status": "{dststat}", "data": {lastdst} , "time_o": "{dtt}", "time_0": "{time0}", "time": "{lastac}", "lat": {lai}, "lon": {loi}, "serie": {listdst}, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ {loi}, {lai} ] ) ),"""
        ftrs.append(str1)
if not ftrs:
    ftrs = ["""( "type": "Feature", "properties": ( "name": "Smoke", "units": "[-]", "Status": null, "data": null , "time_o": null, "time_0": null, "time": null, "lat": 0, "lon": 0, "serie": null, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ 0, 0 ] ) ),"""]
...
file2up = f'{pthg}/pts_adpS.geojson'
f = open(file2up, 'w')
f.write(jsonf)
f.close()
...
#%%
for i, fl in enumerate(reversed(lst2del2)):
    if i > 30:
        # print(fl)
