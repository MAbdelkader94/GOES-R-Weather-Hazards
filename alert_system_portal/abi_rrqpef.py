...
for gp_key, gp_value in gpd_df:
    ...
    if minsecs == 0:
        listtmp = pdd['RRQPE'].to_list()
        
        lasttmp = pdd['RRQPE'].values[-1]#pdd['Temp'].values[-1]
        ...
        str1 = f"""( "type": "Feature", "properties": ( "name": "Rainfall", "units": "[mm/h]", "Status": "{stat}", "data": {lasttmp} , "time_o": "{dtt}", "time_0": "{time0}", "time": "{lastac}", "lat": {lai}, "lon": {loi}, "serie": {listtmp}, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ {loi}, {lai} ] ) ),"""
        ftrs.append(str1)
if not ftrs:
    ftrs = ["""( "type": "Feature", "properties": ( "name": "Rainfall", "units": "[mm/h]", "Status": null, "data": null , "time_o": null, "time_0": null, "time": null, "lat": 0, "lon": 0, "serie": null, "icon_exp": "_", "html_exp": null ), "geometry": ( "type": "Point", "coordinates": [ 0, 0 ] ) ),"""]
#%%
ftrs[-1] = ftrs[-1][:-1]
jsonmd = '\n'.join(ftrs)
...
file2up = f'{pthg}/pts_rrqpef.geojson'
f = open(file2up, 'w')
f.write(jsonf)
f.close()
